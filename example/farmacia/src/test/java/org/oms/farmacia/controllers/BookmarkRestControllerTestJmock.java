...
@WebAppConfiguration
public class BookmarkRestControllerTestJmock {
    ...
    @Before
    public void setUp(){
        bookmarkRepository = context.mock(BookmarkRepository.class);
	...
	context.checking(new Expectations(){{
	    oneOf(accountRepository).findByUsername(userName);
	...
	context.checking(new Expectations(){{
	    ...
	    oneOf(bookmarkRepository.findOne(bookmarkList.get(0).getId()));
	...
	context.checking(new Expectations(){{
	    oneOf(accountRepository).findByUsername(userName);
	    will(returnValue(account));
	    oneOf(bookmarkRepository).findByAccountUsername(userName);
	    will(returnValue(bookmarkList));
	}});
	...
	context.checking(new Expectations(){{
	    ...
	    oneOf(bookmarkRepository).save(bookmarkSent);
	    will(returnValue(bookmarkResponse));
	}});
